...
**Tags:** powershell, windows, shell · **Confidence:** documented · **Price:** 0.00002 ETH on Base (~$0.049)

## Get the fix

- **MCP**: call `get_offer` with `productType: "fix"` and `productId: "ps51-no-pipeline-chain-operators"`. Pay its exact `priceWei`, then call `get_fix` with both `paymentTx` and `paymentOffer`.
