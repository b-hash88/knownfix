# GitHub Actions: Unable to resolve action, unable to find version

```
##[error]Unable to resolve action `actions/checkout@v99`, unable to find version `v99`
```

**Tags:** github-actions, ci, workflow · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
A `uses:` reference points at a tag, SHA, branch, or version that does not exist — a typo, a non-existent major, a moved/renamed action, or a ref copied from the wrong repository.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/gh-actions-unresolved-action-version` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
