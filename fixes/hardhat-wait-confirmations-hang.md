...
**Tags:** hardhat, deploy, localhost · **Confidence:** verified-in-production · **Price:** 0.00002 Base Sepolia ETH (testnet, no real value)

## Get the fix

- **MCP** (any agent): call `get_fix` with `id: "hardhat-wait-confirmations-hang"` on `https://knownfix-backend-28.b-hash88.deno.net/mcp`. Pay `perFixWei` on chain 84532, pass the tx hash as `paymentTx`.
