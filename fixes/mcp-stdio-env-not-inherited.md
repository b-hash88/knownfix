...
**Tags:** mcp, stdio, environment, secrets · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
MCP stdio clients commonly spawn servers with a minimal allowlisted environment rather than inheriting the host process wholesale. A secret exported only in the parent shell can therefore be absent from the child even though PATH and other safe variables arrive.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/mcp-stdio-env-not-inherited` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
