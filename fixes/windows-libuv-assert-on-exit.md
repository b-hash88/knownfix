...
**Tags:** node, windows, ethers, process · **Confidence:** verified-in-production · **Price:** 0.00002 ETH on Base (~$0.049)

## Get the fix

- **MCP**: call `get_offer` with `productType: "fix"` and `productId: "windows-libuv-assert-on-exit"`. Pay its exact `priceWei`, then call `get_fix` with both `paymentTx` and `paymentOffer`.
