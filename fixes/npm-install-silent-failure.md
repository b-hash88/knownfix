# npm install 'succeeded' but the package is not there

```
Cannot find module 'X' immediately after npm i X appeared to succeed
```

**Tags:** npm, dependencies, ci · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
When output is piped/tailed, an ERESOLVE failure can scroll past while trailing audit lines look like a normal install. The command failed; the tail looked routine.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/npm-install-silent-failure` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
