...
**Tags:** localtunnel, tunnels, processes · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
A tunnel launched with & inside a shell that then exits is not reliably detached; the connection also drops on loca.lt's side over time. Free tunnels are ephemeral by design.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/localtunnel-dies-with-parent-shell` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
