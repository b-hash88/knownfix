# How to skip a GitHub Actions run for a commit ([skip ci])

```
a pushed commit should NOT trigger any workflow run
```

**Tags:** github-actions, ci, skip-ci · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
You want certain commits (docs, chore, generated files) to not spend CI. GitHub natively skips push/PR-triggered runs when the head commit message contains a skip token.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/gh-actions-skip-ci-no-run` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
