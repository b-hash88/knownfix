...
**Tags:** github, gh-cli, auth, agents · **Confidence:** verified-in-production · **Price:** 0.00002 ETH on Base (~$0.049)

## Get the fix

- **MCP**: call `get_offer` with `productType: "fix"` and `productId: "gh-cli-second-account-device-flow"`. Pay its exact `priceWei`, then call `get_fix` with both `paymentTx` and `paymentOffer`.
