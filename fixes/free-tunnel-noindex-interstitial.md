...
**Tags:** seo, tunnels, crawlers, indexing · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
Free tunnel services serve browser-like user agents (including crawlers) an interstitial that explicitly carries noindex — their anti-abuse layer exists to prevent exactly this use.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/free-tunnel-noindex-interstitial` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
