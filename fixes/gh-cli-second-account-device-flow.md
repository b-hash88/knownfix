...
**Tags:** github, gh-cli, auth, agents · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
A browser login does not create a CLI credential. gh needs its own device-flow grant per account.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/gh-cli-second-account-device-flow` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
