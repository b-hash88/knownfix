...
**Tags:** github-pages, deploy, ci · **Confidence:** verified-in-production · **Price:** $0.05 USDC on Base, or signed exact ETH

## Free diagnosis
Pages rebuilds asynchronously after each push; the CDN serves the old tree until the build lands.
...
- **HTTP**: `GET https://knownfix-backend-28.b-hash88.deno.net/fix/gh-pages-deploy-lag` with no proof headers returns the same purchase-ready 402; after payment, retry with both `x-payment-tx` and `x-payment-offer`.
